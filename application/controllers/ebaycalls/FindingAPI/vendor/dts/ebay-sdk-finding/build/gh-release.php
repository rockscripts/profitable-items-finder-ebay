...
assert(file_exists(__DIR__ . '/artifacts/dts-finding.zip'));
assert(file_exists(__DIR__ . '/artifacts/dts-finding.phar'));
...
$client->post(
    $uploadUrl . '/assets?name=dts-finding.zip',
    array('Content-Type' => 'application/zip'),
    fopen(__DIR__ . '/artifacts/dts-finding.zip', 'r')
)->send();

$client->post(
    $uploadUrl . '/assets?name=dts-finding.phar',
    array('Content-Type' => 'application/phar'),
    fopen(__DIR__ . '/artifacts/dts-finding.phar', 'r')
)->send();
