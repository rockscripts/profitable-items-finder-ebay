...
class Profit extends CI_Controller {
	...
	{	
	    /*echo $this->input->get("sellerID");
	    if($this->input->get("sellerID"))
		$data["userID"] = $this->input->get("sellerID");	
	    else*/
	    $data["userID"] = $userID = "30165847";	
	...
	{		
	  ...
	  echo json_encode($data);
		...
		$userID = 30165847 /* $this->input->post("userID")"gig-games"*/;//economy-games		
		if(!$this->input->post("page"))
			$pageToImport=1;
                
                $response = $this->get_seller_items_for_sale($userID, $pageToImport);
		if(!$this->Profit_model->exist_profitable_seller(array("userID"=>$userID)))
                {
                    $totalPages = ceil($response["body"]->paging->total / 200);
                    $this->Profit_model->import_profitable_seller(array("userID"=>$userID, "totalPages"=>$totalPages,"totalEntries"=>$response["body"]->paging->total));
                }		 			
			for ($i=0;$i<$response["body"]->paging->limit;$i++) 
			{
				...
				if($item->id!=null)
				{
				    $profitable_item = array("Title"=>$item->title,"itemID"=>$item->id,"viewItemURL"=>$item->permalink,"StartPrice"=>$item->price,"totalQTYPurchased"=> 0, "totalMoneyPurchased" => $item->price*$item->sold_quantity, "totalItemsSold" => $item->sold_quantity, "userID"=>$userID);		   
